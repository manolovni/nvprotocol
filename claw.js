...
async function callWithKey(path, opts = {}) {
  ...
  if (!apiKey) {
    console.error(JSON.stringify({
      ...
      message: 'Run: node claw.js subscribe   (x402 payment)\n   or: node claw.js set-key <your_key>\n   or: node claw.js referral-redeem --wallet 0x... --code XXXX',
    }, null, 2));
    process.exit(1);
  }
  ...
  if (res.status === 402) {
    ...
    console.error(JSON.stringify({
      ...
      ...body,
      tip: 'Check balance: node claw.js status\nBuy credits: node claw.js buy-credits --amount 10',
    }, null, 2));
    process.exit(1);
  }
...
const commands = {
  ...
  async 'set-key'(args) {
    ...
    if (!key || !key.startsWith('nva_')) {
      console.error('Usage: node claw.js set-key nva_your_key_here');
      process.exit(1);
    }
    setApiKey(key);
  ...
  async 'packs-info'(args, opts) {
    if (!opts.coin) { console.error('Usage: node claw.js packs-info --coin BTC'); process.exit(1); }
  ...
  async 'referral'(args, opts) {
    if (!opts.wallet) { console.error('Usage: node claw.js referral --wallet 0x...'); process.exit(1); }
    out(await callFree(`/api/claw/referral?wallet=${encodeURIComponent(opts.wallet)}`));
  },

  async 'referral-stats'(args, opts) {
    if (!opts.wallet) { console.error('Usage: node claw.js referral-stats --wallet 0x...'); process.exit(1); }
  ...
  async 'referral-redeem'(args, opts) {
    if (!opts.wallet || !opts.code) {
      console.error('Usage: node claw.js referral-redeem --wallet 0x... --code ABCD1234');
      process.exit(1);
    }
  ...
  async 'buy-credits'(args, opts) {
    const apiKey = getApiKey();
    if (!apiKey) { console.error('Need an active subscription first. Run: node claw.js subscribe'); process.exit(1); }
  ...
  async 'snapshot'(args, opts) {
    if (!opts.coins) { console.error('Usage: node claw.js snapshot --coins BTC,ETH [--indicators RSI_3H30M]'); process.exit(1); }
  ...
  async 'history'(args, opts) {
    if (!opts.coin) { console.error('Usage: node claw.js history --coin BTC [--hours 24] [--indicators RSI_3H30M]'); process.exit(1); }
  ...
  async 'check'(args, opts) {
    if (!opts.coin || !opts.yaml) {
      console.error('Usage: node claw.js check --coin BTC --yaml "path/to/signal.yaml"');
      console.error('   or: echo "yaml..." | node claw.js check --coin BTC --stdin');
      process.exit(1);
    }
    let yamlBody;
    if (opts.yaml) {
    ...
    if (typeof result === 'string' && result.includes('status: scored')) {
      try {
        ...
        if (coinMatch && nameMatch) {
          ...
          console.error(`[claw] Signal saved: ${saved}`);
        }
      } catch {}
    }
  ...
  async 'check-inline'(args, opts) {
    if (!opts.coin || !opts.name || !opts.type || !opts.expr || !opts.exit || !opts.hold) {
      console.error('Usage: node claw.js check-inline --coin BTC --name MY_SIG --type LONG --expr "RSI_3H30M <= 30" --exit "RSI_3H30M >= 70" --hold 48');
      process.exit(1);
    }
  ...
  async 'pack'(args, opts) {
    ...
    if (typeof result === 'string' && !result.startsWith('error')) {
      try {
        ...
        console.error(`[claw] Pack saved: ${saved}`);
      } catch {}
    }
  ...
  async 'backtest'(args, opts) {
    if (!opts.yaml) { console.error('Usage: node claw.js backtest --yaml path/to/strategy.yaml [--days 90]'); process.exit(1); }
  ...
  async 'assemble'(args, opts) {
    if (!opts.yaml) { console.error('Usage: node claw.js assemble --yaml path/to/signals.yaml [--mode normal] [--max 10]'); process.exit(1); }
    const yamlBody = fs.readFileSync(opts.yaml, 'utf8');
    ...
    if (typeof result === 'string' && !result.startsWith('error')) {
      try {
        ...
        if (coinMatch) {
          ...
          console.error(`[claw] Strategy saved: ${saved}`);
        }
      } catch {}
    }
  ...
  async 'portfolio'(args, opts) {
    if (!opts.existing) { console.error('Usage: node claw.js portfolio --existing BTC,SOL [--count 5] [--mode normal] [--allocation fixed]'); process.exit(1); }
