...
function loadStrategies(files, folder) {
  ...
  if (strategies.length === 0) throw new Error('No strategies found. Use --strategy, --strategies, or save strategies via claw.js assemble.');
...
async function main() {
  ...
  if (!token) {
    console.error('[monitor] No API key found. Set NVARENA_API_KEY or run: node claw.js subscribe');
    process.exit(1);
  }
